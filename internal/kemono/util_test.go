...
func TestTime_UnmarshalJSON(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			"string",
			Time{},
			args{[]byte(`"2024-05-02T14:21:02.807702"`)},
			time.Date(2024, time.May, 2, 14, 21, 2, 807702000, time.UTC),
			require.NoError,
		},
		{
			"unix",
			Time{},
			args{[]byte(`1714659663`)},
			time.Date(2024, time.May, 2, 14, 21, 3, 0, time.UTC),
			require.NoError,
		},
	}
	for _, tt := range tests {
